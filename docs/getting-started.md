...
Для ручного прогона без пяти отдельных аккаунтов включите `TEST_GAME_ENABLED=true` только в тестовом `.env`, перезапустите бота и вызовите `/testgame` в пустой тестовой группе. Команда добавит четырёх виртуальных игроков, автоматически подтвердит их роли, ночные действия и голоса; организатор остаётся ручным игроком.
